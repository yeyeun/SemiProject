...
public class CourseController {
	...
	public String list(Model model) { // 코드 전체 정보 중 5개만 추출해서 List형식으로 리턴
		...
		List<Course> filteredList = courseList.stream()
		                                    .filter(course -> !course.getContentid().equals("2717306"))
		...
		try {
			titleInfo = getTitleList(contentIds);
	...
	@GetMapping("/tourlist")
	public Map<String, List<?>> getTitleList(List<String> contentIds) throws IOException {
