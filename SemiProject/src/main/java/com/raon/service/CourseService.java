...
public class CourseService {
    ...
    //여행코스의 subcontentid 문자열->List로 반환
    ...
    //여행코스의 각 여행지 정보 조회
	public void getDetailInfo2(List<String> list, Model model) throws IOException{
		StringBuilder urlBuilder = new StringBuilder(
		...
		urlBuilder.append("&" + URLEncoder.encode("serviceKey", "UTF-8")
				+ "mu4MvO6eUoXAtU8dp%2Bdwyt%2B%2F24GYekx10foLVqNhtViQi60IGrp26ujspnFxZvJc5EZ0UhX99Q6eQ%2FdE2pRwiA"); //서비스키
		...
		JSONArray jsonArray = jsonObject.getJSONArray("item");
		 for (int i = 0; i < jsonArray.length(); i++) {
			 detailInfoList.add(jsonArray.getJSONObject(i).getString("detailInfo"));
	        }
	...
	//여행코스의 각 여행지 정보 조회
    public Map<String, List<?>> getDetailInfo(List<String> list) throws IOException{ //여행코스에 해당하는 여행지 이름 불러오기
    	List<String> titleList = new ArrayList<>();
		List<Integer> titleLength = new ArrayList<Integer>();

		// 각 subcontentId 문자열을 반복하여 처리
		for (String subcontentIdListString : list) {
			int itemCount = 0; // 각 API 호출 이후에 추가된 항목의 개수를 저장
