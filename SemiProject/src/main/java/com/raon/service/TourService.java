...
public class TourService {
	...
	/*
	 * public void getTourImage(HttpServletRequest request, HttpServletResponse
	 * response, Model model)throws IOException { // TODO Auto-generated method stub
	 * List<String> imgUrls = new ArrayList<String>(); String contentId =
	 * request.getParameter("contentid"); StringBuilder urlBuilder = new
	 * StringBuilder("https://apis.data.go.kr/B551011/KorService1/detailImage1");
	 * URL urlBuilder.append("?" + URLEncoder.encode("MobileOS","UTF-8") + "=" +
	 * URLEncoder.encode("ETC", "UTF-8")); 조회시작일자 urlBuilder.append("&" +
	 * URLEncoder.encode("MobileApp","UTF-8") + "=" + URLEncoder.encode("raon",
	 * "UTF-8")); 조회종료일자 urlBuilder.append("&" + URLEncoder.encode("_type","UTF-8")
	 ...
	 * urlBuilder.append("&" + URLEncoder.encode("imageYN","UTF-8") + "=" +
	 * URLEncoder.encode("Y", "UTF-8")); 조회시작일자 urlBuilder.append("&" +
	 * URLEncoder.encode("subImageYN","UTF-8") + "=" + URLEncoder.encode("Y",
	 * "UTF-8")); 조회시작일자 urlBuilder.append("&" +
	 ...
	 * System.out.println(sb.toString()); JSONObject jsonObject = new
	 * JSONObject(sb.toString());
	 * jsonObject=jsonObject.getJSONObject("response").getJSONObject("body").
	 * getJSONObject("items"); JSONArray jsonArray =
	 * jsonObject.getJSONArray("item"); System.out.println(jsonObject.keys()); for
	 * (int i = 0; i < jsonArray.length(); i++) {
	 * imgUrls.add(jsonArray.getJSONObject(i).getString("originimgurl")); } // 이미지
	 * URL을 클라이언트로 전송 model.addAttribute("imgUrls", imgUrls);
	 * 
	 * }
	 */
