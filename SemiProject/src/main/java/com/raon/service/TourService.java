...
import com.raon.domain.TourImageInfo;
...
public class TourService {
	...
	public void getTourImage(HttpServletRequest request, HttpServletResponse response, Model model) throws IOException {
		List<TourImageInfo> imgUrls = new ArrayList<TourImageInfo>();
        ...
        Object items = responseObject.getJSONObject("body").get("items");

        // items가 JSONObject인지 확인
        if (items instanceof JSONObject) {
            JSONObject itemsObject = (JSONObject) items;

            // "item" 키가 있는지 확인
            if (itemsObject.has("item")) {
                Object itemValue = itemsObject.get("item");

                // "item" 키의 값이 JSONArray 또는 JSONObject인지 확인
                if (itemValue instanceof JSONArray) {
                    // JSONArray로 변환하여 처리
                    JSONArray jsonArray = (JSONArray) itemValue;
                    for (int i = 0; i < jsonArray.length(); i++) {
                        imgUrls.add(new TourImageInfo(jsonArray.getJSONObject(i).getString("originimgurl")));
                    }
                    ...
                    imgUrls.add(new TourImageInfo("../../resources/images/tour_none_image.png"));
                }
            } else {
                // "item" 키가 없는 경우
                imgUrls.add(new TourImageInfo("../../resources/images/tour_none_image.png"));
...
//	forslider_1
	...
	public void getBusStation(HttpServletRequest request, HttpServletResponse response, Model model) throws IOException {
	    ...
	    if (items instanceof String) {
	        // "items"가 비어 있는 경우
	        System.out.println("API 응답에 'items'가 비어 있습니다.");
	        // "정보없음"을 리스트에 추가
	        ...
	        if (itemsObj.has("item")) {
	            ...
	            if (item instanceof JSONObject) {
	                // 단일 아이템인 경우
	                JSONObject stationObj = (JSONObject) item;
	        ...
	        // "items"가 JSONObject 또는 String이 아닌 경우
	        System.out.println("API 응답에 'items'가 JSONObject 또는 String이 아닙니다.");
	        // "정보없음"을 리스트에 추가
	...
	public void getAllBus(@Param("nodeid") String nodeid, Model model) throws IOException {
	    ...
	    if (item instanceof JSONArray) {
	        ...
	        for (int i = 0; i < jsonArray.length(); i++) {
	            ...
	            String routeType = busObject.isNull("routetp") ? "지선버스" : busObject.getString("routetp");
	            allbus.add(new finalbus(busObject.getString("endnodenm"),
	                    busObject.getString("routeid"), String.valueOf(busObject.get("routeno")),
	                    routeType, busObject.getString("startnodenm"), -1, -1));
	        }
